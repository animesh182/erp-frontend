...
const EmployeeLogin = () => {
  ...
  function onEmailSubmit(values) {
    ...
    const userExists = employeeDetails.some(employee => employee.email === values.email);

    setExistingUser(userExists);

    
    const matchedEmployee=employeeDetails.find(employee=>employee.email===values.email)
    if(matchedEmployee){
  ...
  function onGoogleEmailSubmit(){
    form.clearErrors();
    if(existingUser)
      window.location.href="/users/login-password"
    else
     window.location.href="/users/email-verify"
    }
    ...
    <div className=" lg:grid grid-cols-2">
      ...
      <div className="flex flex-col justify-center space-y-2 lg:px-20  pb-6">
         <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm text-[#020617] font-medium tracking-tight">Email</FormLabel>
                      <FormControl>
                        <Input placeholder="Enter your work email address" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
        <Button className="w-full text-sm" type="submit">Sign in with Email</Button>
       <br/>
      <div className="flex text-[#64748B] justify-center text-xs ">OR CONTINUE WITH</div>
      <br/>
      <Button className="w-full shadow-md text-sm" variant="outline" type="button" onClick={onGoogleEmailSubmit}><Image src={google} alt="google"/> Sign in with Google</Button>
